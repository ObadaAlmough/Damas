...
class deleveryController extends Controller
{
    ...
    public function show(delevery $delevery, Request $request)
    {
        ...
        return view('dashboard.orders.delevery._table-orders', compact('orders', 'delevery'));
    } //end of show



    public function add_order_delevery(delevery $delevery, order $order)
    {

        $order->update([
            'delevery_id' => $delevery->id,
            'states' => 'delevery'
        ]);
        $orders_Delevery = order::where('delevery_id', $delevery->id)->get();

        return view('dashboard.orders.delevery._table-delevery-order' , compact('orders_Delevery', 'delevery'));
    } //end of show\
